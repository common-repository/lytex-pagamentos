...
class Card_lytex_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title = 'Lytex Pagamentos - Cartão de Crédito';
        $this->method_description = 'Receba pagamentos via Cartão de Crédito';
        $this->supports = array(
            'products'
        );
        ...
        $this->max_parcels      = $this->get_option( 'max_parcels' );
        $this->min_value_parcel = $this->get_option( 'min_value_parcel' );
        $this->duedate          = $this->get_option( 'duedate' );

        // This action hook saves the settings
        add_action('woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
        add_action('woocommerce_thankyou_lytex_card', array($this, 'thankyou_page_lytex'));
        add_action('wp_enqueue_scripts', array( $this, 'payment_scripts' ) );
        if(!has_action('woocommerce_api_lytex', 'webhook')){
            add_action('woocommerce_api_lytex', array($this, 'webhook'));
        }
        ...
        if($this->removesoon == "no"){
            $this->icon = apply_filters('Lytex_woocommerce',  plugins_url('lytex-pagamentos/assets/img/logo-dark.png', plugin_dir_path(__FILE__)));
        }
    ...
    public function lytex_issue_duplicate_ticket($post){
        ...
        if($order->data['payment_method'] == "lytex" && $order->data['status'] == "on-hold"):
            ...
            $api = new Lytex_pagamentos_Card_API();
            ?>
    ...
    protected function lytex_thankyou_page($order_id, $my_account = false){
        ...
        $order = wc_get_order($order_id);
        ob_start();
        include_once dirname( __FILE__ ) . '/templates/order-received-card.php';
        $html = ob_get_clean();
        return $html;
    }
    ...
    public function init_form_fields(){
        $this->form_fields = array(
            ...
            'min_value_parcel' => array(
                'title'       => 'Valor mínimo da parcela',
                'type'        => 'text',
                'description' => 'Informe o valor mínimo de cada parcela. Use 0 para desabilitar esta opçãoInforme o valor mínimo de cada parcela. Use 0 para desabilitar esta opção',
                'default'     => '5',
            ),
            'Client_ID' => array(
                'title'       => 'Client ID',
    ...
    public function process_payment( $order_id ) {
        ...
        elseif($result->error->details[0]->message == true){
            ...
            $PaymentMethodLytex = sanitize_text_field($_POST['lypay_finalPaymentMethod']);
                ...
                $card_data = array(
                    "cpfCnpj" => $_POST['lypay-credit_card__holder-cpf'],
                    "number" => $_POST['lypay-credit_card__number'],
                    "holder" => $_POST['lypay-credit_card__holder-name'],
                    "expiry" => $_POST['lypay-credit_card--expiry'],
                    "cvc" => $_POST['lypay-credit_card--cvv'],
                    "_clientId" => $result->_clientId
                );

                $tokenCard = $api->create_card_token($card_data);
                if ($tokenCard['error']){
                    wc_add_notice($tokenCard['message'], 'error');
                    $api->cancelInvoice($result->_id);
                    return array(
                        'result'   => 'fail',
                        'redirect' => '',
                    );
                }else{
                    $pay = $api->payInvoiceCardToken($result->_id, $tokenCard['token']);
                    if ($pay['error']){
                        wc_add_notice($pay['message'], 'error');
                        $api->cancelInvoice($result->_id);
                        return array(
                            'result'   => 'fail',
                            'redirect' => '',
                        ...
                        return array(
                            'result'   => 'success',
                            'redirect' => $this->get_return_url( $order ) . '&PaymentMethod=credit_card'
                            //'redirect' => ''
                        );
...
//        wp_enqueue_script('jquery-wp');
        ...
        $num_instalments = intdiv( $card_total, max(floatval($this->min_value_parcel), 5));

        include plugin_dir_path(dirname(__FILE__)) . 'lytex-pagamentos/templates/card-checkout.php';
