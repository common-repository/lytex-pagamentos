<?php
    ...
    class WC_lytex_Gateway extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->method_title = 'Lytex Pagamentos - Pix';
            $this->method_description = 'Receba pagamentos via Pix';
            $this->supports = array(
                'products'
            );
            ...
            add_action('woocommerce_thankyou_lytex', array($this, 'thankyou_page_lytex'));
        ...
        protected function lytex_thankyou_page($order_id, $my_account = false){
            ...
            $order = wc_get_order($order_id);
            ob_start();
            include_once dirname( __FILE__ ) . '/templates/order-received.php';
            $html = ob_get_clean();
            return $html;
        }
        ...
        public function process_payment( $order_id ) {
            ...
            elseif($result->error->details[0]->message == true){
                ...
                add_post_meta(intval($order_id), 'CodePix', $result->paymentMethods->pix->qrcode, true);
                add_post_meta(intval($order_id), 'invoices_id', $result->_id, true);
                add_post_meta(intval($order_id), 'PaymentMethod', 'pix', true);
                update_post_meta(intval($order_id), '_payment_method_title', sanitize_text_field(__('PIX - Lytex Pagamentos')));
                $order->update_status( 'on-hold', __( 'Awaiting online payment', 'wc-gateway-lytes' ) );
                return array(
                    'result'   => 'success',
                    'redirect' => $this->get_return_url( $order ) . '&PaymentMethod=pix'
                );
            }
...
//            wp_enqueue_script('jquery-wp');
            ...
            include plugin_dir_path(dirname(__FILE__)) . 'lytex-pagamentos/templates/lytex-checkout.php';
